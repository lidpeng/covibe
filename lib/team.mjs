...
async function teamInit(cwd, args) {
  ...
  if (needsHooks) {
    ...
    settings.hooks.PostToolUse.push({
      matcher: 'Edit|Write',
      hooks: [{ type: 'command', command: 'bash -c \'FILE="${TOOL_INPUT_FILE_PATH:-}"; if [ -n "$FILE" ]; then curl -s -X POST "${HARNESS_SYNC_SERVER:-http://localhost:3456}/broadcast" -H "Content-Type: application/json" -H "X-Covibe-Token: ${HARNESS_SYNC_TOKEN:-}" -d "{\\\"type\\\":\\\"editing\\\",\\\"name\\\":\\\"${HARNESS_SYNC_USER:-$(whoami)}\\\",\\\"file\\\":\\\"$FILE\\\"}" > /dev/null 2>&1; fi\'' }]
    });
