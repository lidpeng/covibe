...
export async function auditFix(cwd) {
  console.log('🔧 自动修复 harness 问题...\n');

  const { existsSync: ex, mkdirSync: mk } = await import('fs');
  const { join: j } = await import('path');
  let fixed = 0;

  // K1: CLAUDE.md 不存在 → 运行 init
  if (!ex(j(cwd, 'CLAUDE.md')) && !ex(j(cwd, '.claude/CLAUDE.md'))) {
    const { init } = await import('./init.mjs');
    await init(cwd, []);
    fixed++;
  }

  // P1: settings.local.json 不存在
  const claudeDir = j(cwd, '.claude');
  if (!ex(claudeDir)) mk(claudeDir, { recursive: true });
  const localSettings = j(claudeDir, 'settings.local.json');
  if (!ex(localSettings)) {
  ...
  if (fixed === 0) {
    console.log('  无需修复，一切看起来都很好！🎵');
  } else {
    console.log(`\n  修复了 ${fixed} 个问题。再跑一次 covibe audit 检查 🎸`);
  }
}
