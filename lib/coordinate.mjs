...
export async function coordinate(cwd, args) {
  ...
  for (const f of readdirSync(profileDir).filter(f => f.endsWith('.json'))) {
    try { profiles.push(JSON.parse(readFileSync(join(profileDir, f), 'utf8'))); } catch {}
  }
  ...
  for (const p of profiles) {
    const skills = p.skills?.primary || p.skills || [];
    const skillStr = Array.isArray(skills) ? skills.join(', ') : Object.values(skills).flat().join(', ');
