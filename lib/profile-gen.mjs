...
export async function profileGen(cwd, args) {
  ...
  try {
    const raw = execSync(
      `git log --author="${name}" --since="90 days ago" --name-only --pretty=format:"" -- . | grep -v '^$' | sed 's|/[^/]*$||' | sort | uniq -c | sort -rn | head -8`,
      { cwd, encoding: 'utf8' }
    ).trim();
  ...
  try {
    const raw = execSync(
      `git log --author="${name}" --since="90 days ago" --name-only --pretty=format:"" -- . | grep -v '^$' | grep -oE '\\.[^.]+$' | sort | uniq -c | sort -rn`,
      { cwd, encoding: 'utf8' }
    ).trim();
  ...
  try {
    totalCommits = parseInt(execSync(
      `git log --author="${name}" --since="90 days ago" --oneline | wc -l`,
      { cwd, encoding: 'utf8' }
    ).trim());
  } catch {}
  ...
  try {
    const raw = execSync(
      `git log --author="${name}" --since="30 days ago" --format="%H" | sort | uniq -c | sort -rn | head -1`,
      { cwd, encoding: 'utf8' }
    ).trim();
