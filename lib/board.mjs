...
function loadBoard(cwd) {
  ...
  try { return JSON.parse(readFileSync(p, 'utf8')); } catch { return { version: '1.0', tasks: [], id_counter: 0 }; }
}
...
async function boardVibe(cwd, args) {
  ...
  if (!id) { console.log('用法: covibe board vibe <id>'); return; }
...
async function boardDone(cwd, args) {
  const id = parseInt(args[0]);
  if (!id) { console.log('用法: covibe board done <id>'); return; }
  const user = process.env.HARNESS_SYNC_USER || 'anonymous';
  const board = loadBoard(cwd);
  const task = board.tasks.find(t => t.id === id);
  if (!task) { console.log(`任务 #${id} 不存在`); return; }
...
async function boardBlock(cwd, args) {
  const id = parseInt(args[0]);
  if (!id) { console.log('用法: covibe board block <id>'); return; }
  const board = loadBoard(cwd);
  const task = board.tasks.find(t => t.id === id);
  if (!task) { console.log(`任务 #${id} 不存在`); return; }
  task.status = 'blocked';
...
async function boardDrop(cwd, args) {
  const id = parseInt(args[0]);
  if (!id) { console.log('用法: covibe board drop <id>'); return; }
  const board = loadBoard(cwd);
...
async function boardAssign(cwd, args) {
  ...
  if (!id || !assignee) { console.log('用法: covibe board assign <id> <人名>'); return; }
  const board = loadBoard(cwd);
  const task = board.tasks.find(t => t.id === id);
  if (!task) { console.log(`任务 #${id} 不存在`); return; }
