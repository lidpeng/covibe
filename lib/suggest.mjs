...
async function sendSuggestion(cwd, args) {
  ...
  if (server) {
    ...
    } catch (e) { console.warn('suggest: failed to broadcast suggestion:', e.message); }
  }
}
...
async function listSuggestions(cwd, args) {
  ...
  for (const f of files) {
    ...
    } catch (e) { console.warn(`suggest: failed to parse ${f}:`, e.message); }
  }
    ...
    try {
      ...
      if (s.id.endsWith(idFragment) && s.status === 'pending') {
        ...
        if (server) {
          ...
          } catch (e) { console.warn('suggest: failed to broadcast response:', e.message); }
        }
        return;
      }
    } catch (e) { console.warn(`suggest: failed to process ${f}:`, e.message); }
  }
